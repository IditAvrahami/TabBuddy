...
from backend.test.conftest import get_db_count, get_db_drug  # noqa: E402


def test_add_drug_success(db_session: Session, test_client: TestClient) -> None:
    payload = {
        "name": "Paracetamol",
        "kind": "pill",
        "amount_per_dose": 2,
...
def test_add_drug_duplicate(db_session: Session, test_client: TestClient) -> None:
    payload = {
        "name": "Paracetamol",
        "kind": "pill",
        "amount_per_dose": 2,
...
def test_get_all_drugs(db_session: Session, test_client: TestClient) -> None:
    # Add a drug first
    payload = {
        "name": "Paracetamol",
        "kind": "pill",
...
def test_update_drug_not_found(db_session: Session, test_client: TestClient) -> None:
    missing = {
        "name": "NonExistent",
        "kind": "liquid",
        "amount_per_dose": 10,
...
def test_delete_drug_success(db_session: Session, test_client: TestClient) -> None:
...
def test_delete_drug_not_found(db_session: Session, test_client: TestClient) -> None:
    # Check DB state before failed deletion
    count_before = get_db_count(db_session)
    assert (
        get_db_drug(db_session, "Paracetamol") is None
    )  # Already deleted in previous test
