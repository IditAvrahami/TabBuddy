from fastapi.testclient import TestClient

# Test data for all three meals
TEST_MEALS = [
    {"meal_name": "breakfast", "base_time": "08:00"},
    {"meal_name": "lunch", "base_time": "13:00"},
    {"meal_name": "dinner", "base_time": "19:00"},
]


def test_get_meal_schedules_empty(test_client: TestClient) -> None:
    """Test getting meal schedules when none exist"""
    response = test_client.get("/meal-schedules")
    assert response.status_code == 200
    assert response.json() == []


def test_create_all_meals(test_client: TestClient) -> None:
    """Test creating all three meal schedules (breakfast, lunch, dinner)"""
    for meal in TEST_MEALS:
        response = test_client.post("/meal-schedules", json=meal)
        assert response.status_code == 200
        data = response.json()
        assert data["meal_name"] == meal["meal_name"]
        assert data["base_time"] == meal["base_time"]

    # Verify all meals were created
    response = test_client.get("/meal-schedules")
    assert response.status_code == 200
    meals = response.json()
    assert len(meals) == 3

    # Check that all meals exist and are in correct order
    meal_names = [meal["meal_name"] for meal in meals]
    assert meal_names == ["breakfast", "lunch", "dinner"]


def test_create_duplicate_meal_schedule(test_client: TestClient) -> None:
    """Test creating a duplicate meal schedule fails"""
    # Create breakfast first
    test_client.post("/meal-schedules", json=TEST_MEALS[0])

    # Try to create duplicate breakfast
    response = test_client.post("/meal-schedules", json=TEST_MEALS[0])
    assert response.status_code == 400
    assert "Meal schedule already exists" in response.json()["detail"]


def test_get_all_meal_schedules(test_client: TestClient) -> None:
    """Test getting all meal schedules after creating them"""
    # Create all meals
    for meal in TEST_MEALS:
        test_client.post("/meal-schedules", json=meal)

    response = test_client.get("/meal-schedules")
    assert response.status_code == 200
    meals = response.json()
    assert len(meals) == 3

    # Verify each meal has correct data
    for i, expected_meal in enumerate(TEST_MEALS):
        actual_meal = meals[i]
        assert actual_meal["meal_name"] == expected_meal["meal_name"]
        assert actual_meal["base_time"] == expected_meal["base_time"]
        assert "id" in actual_meal
        assert "created_at" in actual_meal


def test_update_breakfast_time(test_client: TestClient) -> None:
    """Test updating breakfast time specifically"""
    # Create all meals
    for meal in TEST_MEALS:
        test_client.post("/meal-schedules", json=meal)

    # Update only breakfast time
    update_data = {"base_time": "07:30"}
    response = test_client.put("/meal-schedules/breakfast", json=update_data)
    assert response.status_code == 200
    data = response.json()
    assert data["meal_name"] == "breakfast"
    assert data["base_time"] == "07:30"

    # Verify only breakfast was updated
    response = test_client.get("/meal-schedules")
    meals = response.json()

    breakfast = next(meal for meal in meals if meal["meal_name"] == "breakfast")
    lunch = next(meal for meal in meals if meal["meal_name"] == "lunch")
    dinner = next(meal for meal in meals if meal["meal_name"] == "dinner")

    assert breakfast["base_time"] == "07:30"  # Updated
    assert lunch["base_time"] == "13:00"  # Unchanged
    assert dinner["base_time"] == "19:00"  # Unchanged


def test_update_lunch_time(test_client: TestClient) -> None:
    """Test updating lunch time while keeping others unchanged"""
    # Create all meals
    for meal in TEST_MEALS:
        test_client.post("/meal-schedules", json=meal)

    # Update only lunch time
    update_data = {"base_time": "12:30"}  # Change lunch time
    response = test_client.put("/meal-schedules/lunch", json=update_data)
    assert response.status_code == 200
    data = response.json()
    assert data["meal_name"] == "lunch"
    assert data["base_time"] == "12:30"

    # Verify only lunch was updated
    response = test_client.get("/meal-schedules")
    meals = response.json()

    breakfast = next(meal for meal in meals if meal["meal_name"] == "breakfast")
    lunch = next(meal for meal in meals if meal["meal_name"] == "lunch")
    dinner = next(meal for meal in meals if meal["meal_name"] == "dinner")

    assert breakfast["base_time"] == "08:00"  # Unchanged
    assert lunch["base_time"] == "12:30"  # Updated
    assert dinner["base_time"] == "19:00"  # Unchanged


def test_update_dinner_time(test_client: TestClient) -> None:
    """Test updating dinner time"""
    # Create all meals
    for meal in TEST_MEALS:
        test_client.post("/meal-schedules", json=meal)

    # Update dinner time
    update_data = {"base_time": "20:30"}
    response = test_client.put("/meal-schedules/dinner", json=update_data)
    assert response.status_code == 200
    data = response.json()
    assert data["meal_name"] == "dinner"
    assert data["base_time"] == "20:30"

    # Verify dinner was updated correctly
    response = test_client.get("/meal-schedules")
    meals = response.json()

    dinner = next(meal for meal in meals if meal["meal_name"] == "dinner")
    assert dinner["base_time"] == "20:30"


def test_update_nonexistent_meal_schedule(test_client: TestClient) -> None:
    """Test updating a non-existent meal schedule fails"""
    update_data = {"base_time": "20:00"}
    response = test_client.put("/meal-schedules/nonexistent", json=update_data)
    assert response.status_code == 404
    assert "Meal schedule not found" in response.json()["detail"]


def test_delete_breakfast_keep_others(test_client: TestClient) -> None:
    """Test deleting breakfast while keeping lunch and dinner"""
    # Create all meals
    for meal in TEST_MEALS:
        test_client.post("/meal-schedules", json=meal)

    # Delete only breakfast
    response = test_client.delete("/meal-schedules/breakfast")
    assert response.status_code == 200
    data = response.json()
    assert data["meal_name"] == "breakfast"

    # Verify only breakfast was deleted
    response = test_client.get("/meal-schedules")
    meals = response.json()
    assert len(meals) == 2

    meal_names = [meal["meal_name"] for meal in meals]
    assert "breakfast" not in meal_names
    assert "lunch" in meal_names
    assert "dinner" in meal_names


def test_delete_nonexistent_meal_schedule(test_client: TestClient) -> None:
    """Test deleting a non-existent meal schedule fails"""
    response = test_client.delete("/meal-schedules/nonexistent")
    assert response.status_code == 404
    assert "Meal schedule not found" in response.json()["detail"]


def test_invalid_time_format(test_client: TestClient) -> None:
    """Test that invalid time format is rejected"""
    meal_data = {"meal_name": "breakfast", "base_time": "25:00"}  # Invalid time
    response = test_client.post("/meal-schedules", json=meal_data)
    assert response.status_code == 400
    assert "Invalid time format" in response.json()["detail"]


def test_realistic_meal_management_workflow(test_client: TestClient) -> None:
    """Test a realistic workflow: create all meals, update some, delete one"""
    # Step 1: Create all three meals
    for meal in TEST_MEALS:
        response = test_client.post("/meal-schedules", json=meal)
        assert response.status_code == 200

    # Step 2: Verify all meals exist
    response = test_client.get("/meal-schedules")
    meals = response.json()
    assert len(meals) == 3

    # Step 3: Update breakfast to earlier time
    response = test_client.put("/meal-schedules/breakfast", json={"base_time": "07:00"})
    assert response.status_code == 200

    # Step 4: Update lunch to different time
    response = test_client.put("/meal-schedules/lunch", json={"base_time": "12:30"})
    assert response.status_code == 200

    # Step 5: Update dinner to later time
    response = test_client.put("/meal-schedules/dinner", json={"base_time": "20:00"})
    assert response.status_code == 200

    # Step 6: Verify all changes
    response = test_client.get("/meal-schedules")
    meals = response.json()

    breakfast = next(meal for meal in meals if meal["meal_name"] == "breakfast")
    lunch = next(meal for meal in meals if meal["meal_name"] == "lunch")
    dinner = next(meal for meal in meals if meal["meal_name"] == "dinner")

    assert breakfast["base_time"] == "07:00"
    assert lunch["base_time"] == "12:30"
    assert dinner["base_time"] == "20:00"

    # Step 7: Delete lunch
    response = test_client.delete("/meal-schedules/lunch")
    assert response.status_code == 200

    # Step 8: Verify final state
    response = test_client.get("/meal-schedules")
    meals = response.json()
    assert len(meals) == 2

    meal_names = [meal["meal_name"] for meal in meals]
    assert "breakfast" in meal_names
    assert "lunch" not in meal_names
    assert "dinner" in meal_names
