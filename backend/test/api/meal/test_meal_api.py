...
def test_create_duplicate_meal_schedule(test_client):
...
def test_get_all_meal_schedules(test_client):
...
def test_update_breakfast_time(test_client):
...
def test_update_lunch_time(test_client):
...
def test_update_dinner_time(test_client):
...
def test_update_nonexistent_meal_schedule(test_client):
...
def test_delete_breakfast_keep_others(test_client):
...
def test_realistic_meal_management_workflow(test_client):
    ...
    response = test_client.put("/meal-schedules/breakfast", json={
        "base_time": "07:00"
    })
    assert response.status_code == 200
    
    # Step 4: Update lunch to different time
    response = test_client.put("/meal-schedules/lunch", json={
        "base_time": "12:30"
    })
    assert response.status_code == 200
    
    # Step 5: Update dinner to later time
    response = test_client.put("/meal-schedules/dinner", json={
        "base_time": "20:00"
    })
    assert response.status_code == 200
